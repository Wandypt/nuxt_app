...
const subscribe = async () => {
  // 创建订单
  const { ok, data } = await http<IResult>('post', '/api/order', { courseId: id })
...
<template>
  <section class="rounded bg-white flex p-5 my-2">
    <NImage
      ...
      class="rounded w-[180px] h-[210px] mr-5"
    />
  ...
  <NGrid :x-gap="20" class="pt-2">
    ...
    <NGridItem :span="6">
      <section class="bg-white shawdow rounded mb-5">
        ...
        <div class="p-3">
          <Course v-for="prod in data?.data.recommend" :key="prod.id" :data="prod" />
        </div>
      </section>
    </NGridItem>
  </NGrid>
